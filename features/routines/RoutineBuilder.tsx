...
import { DailyActivityGoal, ActivityCategory, ActivityComponent } from '../../types';
import { useAppContext } from '../../contexts/AppContext';
import { Card } from '../../components/Card';
import { Button } from '../../components/Button';
import { Modal } from '../../components/Modal';
import { ACTIVITY_CATEGORIES_OPTIONS } from '../../constants';
...
export const RoutineBuilder: React.FC = () => {
  ...
  const openEditModal = (target: DailyActivityGoal) => {
    ...
    setHabitMinMinutesTotal(target.minMinutesTotal);
    setHabitOptimalMinutesTotal(target.optimalMinutesTotal);
  ...
  const handleModalSubmit = () => {
    ...
    const newHabitData = {
        ...
        minMinutesTotal: habitMinMinutesTotal,
        optimalMinutesTotal: habitOptimalMinutesTotal,
    };
  ...
  const getHabitDisplayTime = (habit: DailyActivityGoal): string => {
    if (habit.optimalMinutesTotal > 0 && habit.minMinutesTotal > 0 && habit.minMinutesTotal < habit.optimalMinutesTotal) {
      return `${habit.minMinutesTotal}-${habit.optimalMinutesTotal} min totales`;
    }
    if (habit.optimalMinutesTotal > 0) {
      return `${habit.optimalMinutesTotal} min óptimos totales`;
    }
    if (habit.minMinutesTotal > 0) {
      return `Mín: ${habit.minMinutesTotal} min totales`;
    }
