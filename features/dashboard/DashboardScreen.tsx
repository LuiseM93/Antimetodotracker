...
import { CombinedStatusCard } from './CombinedStatusCard'; // NEW
import { STAGE_DETAILS, AVAILABLE_LANGUAGES_FOR_LEARNING } from '../../constants';
import { ActivityLogEntry, AntimethodStage, Language, AppView, DailyActivityGoal, DashboardCardDisplayMode } from '../../types';
...
const calculateStreak = (logs: ActivityLogEntry[], language: Language | undefined): number => {
    if (!language || logs.length === 0) return 0;

    const langLogs = logs.filter(log => log.language === language);
    if (langLogs.length === 0) return 0;

    const sortedDates = [...new Set(langLogs.map(log => log.date))].sort((a,b) => new Date(b).getTime() - new Date(a).getTime());
    ...
    const diffFromToday = (today.getTime() - firstLogDate.getTime()) / (1000 * 3600 * 24);
    ...
    streak = 1;
    let currentExpectedDate = new Date(firstLogDate);
    ...
    for (let i = 1; i < sortedDates.length; i++) {
        ...
        if (logDate.getTime() === currentExpectedDate.getTime()) {
            streak++;
            currentExpectedDate.setDate(currentExpectedDate.getDate() - 1);
        } else {
            break; 
...
export const DashboardScreen: React.FC = () => {
  ...
  const currentStreak = useMemo(() => {
    return calculateStreak(activityLogs, userProfile?.primaryLanguage);
  ...
  const dashboardCardMode = userProfile.dashboardCardDisplayMode || 'both';
    ...
    <div className="p-4 sm:p-6 space-y-6">
      <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center space-y-3 sm:space-y-0">
        <div className="flex-grow">
          <h1 className={`text-3xl font-poppins font-bold text-[var(--color-primary)]`}>
            {userProfile.name ? `Dashboard de ${userProfile.name}` : 'Dashboard'}
          </h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="md:col-span-1 space-y-6">
          {dashboardCardMode === 'both' && (
            <>
              <StreakCard currentStreak={currentStreak} />
              <HabitHealthCard healthPercentage={habitHealthPercentage} />
            </>
          )}
          {dashboardCardMode === 'streak_only' && <StreakCard currentStreak={currentStreak} />}
          {dashboardCardMode === 'health_only' && <HabitHealthCard healthPercentage={habitHealthPercentage} />}
          {dashboardCardMode === 'combined' && <CombinedStatusCard currentStreak={currentStreak} habitHealthPercentage={habitHealthPercentage} />}
            ...
            {recentActivities.map(log => (
              <li key={log.id} className={`p-3 bg-[var(--color-card-bg)] rounded-md shadow-sm border border-[var(--color-border-light)] flex justify-between items-center hover:shadow-md transition-shadow`}>
                <div>
                  {log.customTitle ? (
                    <>
                      <span className={`font-semibold text-lg text-[var(--color-primary)]`}>{log.customTitle}</span>
                      <p className={`text-sm text-[var(--color-secondary)]`}>{log.subActivity} ({log.category})</p>
                    </>
                  ) : (
                    <span className={`font-semibold text-lg text-[var(--color-primary)]`}>{log.subActivity} ({log.category})</span>
                  )}
                  <p className={`text-sm text-[var(--color-text-light)] mt-1`}>
                    {formatDurationFromMinutes(log.durationMinutes, 'hhmmss')} - {formatDisplayDate(log.date)}
                  </p>
                ...
                <div className="flex space-x-1">
                    <Button 
                        ...
                        aria-label={`Editar ${log.customTitle || log.subActivity}`}
                        className="p-2"
                    >
                    ...
                    <Button 
                        ...
                        aria-label={`Re-log ${log.customTitle || log.subActivity}`}
                        className="p-2"
                    >
