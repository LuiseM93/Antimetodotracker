...
export const ActivityReportCharts: React.FC<ActivityReportChartsProps> = ({ logs, selectedLanguage }) => {
  ...
  const totalTimeData = useMemo(() => {
    ...
    filteredLogs.forEach(log => {
      ...
      aggregatedData[key] = (aggregatedData[key] || 0) + log.durationSeconds;
    });
  ...
  const activityBreakdownData = useMemo(() => {
    ...
    filteredLogs.forEach(log => {
      categoryData[log.category] = (categoryData[log.category] || 0) + log.durationSeconds;
    });
  ...
  const skillBreakdownData = useMemo(() => {
    ...
    filteredLogs.forEach(log => {
      ...
      if (activityDetail?.skill) {
        skillData[activityDetail.skill] = (skillData[activityDetail.skill] || 0) + log.durationSeconds;
          ...
          switch(activityDetail.category) {
              ...
              case ActivityCategory.PASSIVE_IMMERSION:
                  skillData[Skill.LISTENING] += log.durationSeconds; 
                  break;
              case ActivityCategory.ACTIVE_STUDY:
                  skillData[Skill.STUDY] += log.durationSeconds;
                  break;
              case ActivityCategory.PRODUCTION: 
                  skillData[Skill.SPEAKING] += log.durationSeconds;
                  break;
          }
      }
    });
          ...
          {activityBreakdownData.length > 0 ? (
            <ResponsiveContainer width="100%" height={300}>
              <PieChart>
                ...
                <Tooltip {...tooltipProps} formatter={(value: number, name: string) => [`${(value/3600).toFixed(1)} horas (${((value / filteredLogs.reduce((acc, log) => acc + log.durationSeconds, 0)) * 100).toFixed(1)}%)`, name]} />
