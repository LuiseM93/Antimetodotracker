...
export const DashboardScreen: React.FC = () => {
  ...
  const totalTimeTodayString = useMemo(() => {
    const totalSeconds = todaysLogsForPrimaryLang.reduce((sum, log) => sum + log.durationSeconds, 0);
              ...
              <li key={log.id} className={`p-3 bg-[var(--color-card-bg)] rounded-md shadow-sm border border-[var(--color-border-light)] flex justify-between items-center hover:shadow-md transition-shadow`}>
                <div>
                  ...
                  <p className={`text-sm text-[var(--color-text-light)] mt-1`}>
                    {formatDurationFromSeconds(log.durationSeconds, 'hhmmss')} - {formatDisplayDate(log.date)}
                  </p>
                ...
                <div className="flex space-x-1">
                    <Button 
                        ...
                        aria-label={`Editar ${log.customTitle || log.sub_activity}`}
                        className="p-2"
                    >
                    ...
                    <Button 
                        ...
                        aria-label={`Re-log ${log.customTitle || log.sub_activity}`}
                        className="p-2"
                    >
