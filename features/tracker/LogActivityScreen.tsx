...
import { Language, ActivityCategory, ActivityLogEntry, AppView, TimerMode, ActivityDetailType, Skill } from '../../types';
...
interface LocationState {
  reLogData?: Partial<ActivityLogEntry> & { language?: Language, durationSeconds?: number };
}
...
export const LogActivityScreen: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (currentPathState?.reLogData) {
        ...
        let reLogTimerDurationMins = Math.round((reLogData.durationSeconds ?? defaultDurationSeconds) / 60);
        resetTimersState(reLogTimerDurationMins);
        setTimerDisplayMode(reLogData.durationSeconds && reLogData.durationSeconds > 0 ? 'countdown' : (userProfile?.defaultLogTimerMode === 'countdown' ? 'countdown' : 'stopwatch'));
        ...
        if (logToEdit) {
            ...
            setManualForm({
                ...
                durationMinutes: Math.round(logToEdit.durationSeconds / 60),
  ...
  const handleSaveActivity = async () => {
    ...
    const logEntryData: Omit<ActivityLogEntry, 'id' | 'user_id' | 'created_at'> = {
      ...
      durationSeconds: durationToSaveSeconds,
      date: dateToSave,
      startTime: timeToSave || null,
  ...
  const handleSaveManualLog = async () => {
    ...
    const logEntryData: Omit<ActivityLogEntry, 'id' | 'user_id' | 'created_at'> = {
      ...
      customTitle: manualForm.customTitle.trim() || null,
    ...
    <div className="flex flex-col h-screen bg-[var(--color-app-bg)]">
      ...
      <div className="flex-grow overflow-y-auto p-4 sm:p-6 space-y-4 flex flex-col items-center">
        ...
        <div className="w-full max-w-md">
            ...
            <select
                ...
                disabled={(userProfile?.learningLanguages.length ?? 0) === 0}
            >
              {(userProfile?.learningLanguages && userProfile.learningLanguages.length > 0) ? (
                ...
                <option disabled>Añade un idioma en Configuración</option>
              )}
            </select>
        </div>
        ...
        <div className="w-full max-w-md">
            ...
            {selectedCategory && (
                <p className="text-xs text-right text-[var(--color-text-light)] mt-1">Categoría: {selectedCategory}</p>
            )}
        </div>
        ...
        <div className="w-full max-w-md">
            ...
            <input
                ...
                placeholder="Ej: Vi 'La Casa de Papel' T1:E1"
                className={inputBaseStyle}
            />
        </div>
        
        {/* Timer Mode Toggle */}
        <div className="flex items-center justify-center p-1 bg-[var(--color-light-purple)] rounded-lg w-full max-w-md">
            <Button
                variant={timerDisplayMode === 'stopwatch' ? 'primary' : 'ghost'}
            ...
            <Button
                variant={timerDisplayMode === 'countdown' ? 'primary' : 'ghost'}
        ...
        <div className="w-full max-w-md space-y-2 mt-4">
            <Button
                ...
                size="lg"
                className="w-full"
                disabled={isStopwatchRunning || isCountdownRunning || !selectedCategory}
            >
                Guardar Actividad
            </Button>
            <Button
                onClick={() => {
                    setIsManualLogModalOpen(true);
                    setManualForm(prev => ({ ...prev, language: currentLanguageForLog }));
                }}
                variant="ghost"
                className="w-full"
            >
      ...
      {isManualLogModalOpen && (
        <Modal
          ...
          title={isEditing ? 'Editar Registro Manual' : 'Registrar Actividad Manualmente'}
          size="md"
        >
          <div className="space-y-4">
            <div>
              ...
              <select
                ...
                onChange={(e) => setManualForm(prev => ({...prev, language: e.target.value as Language}))}
                className={inputBaseStyle}
              >
                {(userProfile?.learningLanguages || AVAILABLE_LANGUAGES_FOR_LEARNING).map(lang => (
            ...
            <div>
              <label className="block text-sm font-medium text-[var(--color-text-main)]">Actividad:</label>
              <Button variant="outline" onClick={() => setIsSelectActivityModalOpenForManualLog(true)} className="w-full justify-start text-left mt-1">
                {manualForm.sub_activity || 'Seleccionar actividad...'}
              </Button>
            </div>
            <div>
              <label htmlFor="manual-title" className="block text-sm font-medium text-[var(--color-text-main)]">Título Personalizado (Opcional):</label>
              <input id="manual-title" type="text" value={manualForm.customTitle} onChange={e => setManualForm(p => ({...p, customTitle: e.target.value}))} className={inputBaseStyle} />
            </div>
            <div className="grid grid-cols-2 gap-4">
                <div>
                    <label htmlFor="manual-date" className="block text-sm font-medium text-[var(--color-text-main)]">Fecha:</label>
                    <input id="manual-date" type="date" value={manualForm.date} onChange={e => setManualForm(p => ({...p, date: e.target.value}))} className={inputBaseStyle} />
                </div>
                <div>
                    <label htmlFor="manual-time" className="block text-sm font-medium text-[var(--color-text-main)]">Hora Inicio (Opcional):</label>
                    <input id="manual-time" type="time" value={manualForm.startTime} onChange={e => setManualForm(p => ({...p, startTime: e.target.value}))} className={inputBaseStyle} />
                </div>
            </div>
            <div>
              <label htmlFor="manual-duration" className="block text-sm font-medium text-[var(--color-text-main)]">Duración (minutos):</label>
              <input id="manual-duration" type="number" value={manualForm.durationMinutes} onChange={e => setManualForm(p => ({...p, durationMinutes: Number(e.target.value) || 0}))} min="1" className={inputBaseStyle} />
            </div>
            <div>
            ...
            <div className={`flex ${isEditing ? 'justify-between' : 'justify-end'} space-x-2 pt-2`}>
              {isEditing && (
              ...
              <div className="flex space-x-2">
                ...
                <Button variant="primary" onClick={handleSaveManualLog}>{isEditing ? 'Guardar Cambios' : 'Guardar Registro'}</Button>
              </div>
            </div>
          </div>
        </Modal>
