    ...
    useEffect(() => {
        const fetchFeed = async () => {
            ...
            if (rpcError) {
                ...
                // The RPC function returns profile data nested in a 'profiles' JSONB object now
                const formattedData = data.map((item: any) => ({
                    ...item,
                    profiles: item.profiles // The RPC function should already format this correctly
                }));
                setFeedItems(formattedData as FeedItem[]);
            }
            setLoading(false);
        };
