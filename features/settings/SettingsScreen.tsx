...
import { AppDataExport, Language, ActivityCategory, ActivityLogEntry, TimerMode, UserProfile, AppView, ActivityDetailType, DashboardCardDisplayMode } from '../../types';
import { ArrowDownTrayIcon } from '../../components/icons/ArrowDownTrayIcon';
import { ArrowUpTrayIcon } from '../../components/icons/ArrowUpTrayIcon';
import { TrashIcon } from '../../components/icons/TrashIcon';
import { AVAILABLE_LANGUAGES_FOR_LEARNING, ANTIMETHOD_ACTIVITIES_DETAILS, DEFAULT_DASHBOARD_CARD_DISPLAY_MODE } from '../../constants';
...
export const SettingsScreen: React.FC = () => {
  ...
  const [dashboardCardMode, setDashboardCardMode] = useState<DashboardCardDisplayMode>(userProfile?.dashboardCardDisplayMode || DEFAULT_DASHBOARD_CARD_DISPLAY_MODE);
  ...
  useEffect(() => {
    if (userProfile) {
        ...
        setDashboardCardMode(userProfile.dashboardCardDisplayMode || DEFAULT_DASHBOARD_CARD_DISPLAY_MODE);
    }
  }, [userProfile]);
  ...
  const handlePreferencesSave = () => {
    if (userProfile) {
        ...
        if (currentLearningLanguages.length > 0 && !currentLearningLanguages.includes(primaryLanguage)) {
            finalPrimaryLang = currentLearningLanguages[0]; 
        } else if (currentLearningLanguages.length === 0) {
            finalPrimaryLang = AVAILABLE_LANGUAGES_FOR_LEARNING[0] as Language;
        }
        ...
        const updates: Partial<UserProfile> = {
            ...
            dashboardCardDisplayMode: dashboardCardMode,
        };
    ...
    <div className="p-4 sm:p-6 space-y-6">
      ...
      <Card title="Preferencias de Registro y Perfil">
        <div className="space-y-6">
            ...
            {currentLearningLanguages.length > 0 ? (
              <ul className="space-y-2 mb-3">
                {currentLearningLanguages.map(lang => (
                  <li key={lang} className="flex justify-between items-center p-2 bg-gray-50 dark:bg-gray-700/30 rounded text-sm">
                    ...
                    <Button 
                      ...
                      className="text-red-500 hover:bg-red-100 dark:hover:bg-red-800/30"
          ...
          <div>
            <label htmlFor="dashboardCardMode" className={`block text-sm font-medium text-[var(--color-text-main)]`}>
              Tarjetas de Resumen en Dashboard:
            </label>
            <select
              id="dashboardCardMode"
              value={dashboardCardMode}
              onChange={(e) => setDashboardCardMode(e.target.value as DashboardCardDisplayMode)}
              className={inputBaseStyle}
            >
              <option value="learning_days_and_health">Ambas (Salud y Días, separadas)</option>
              <option value="health_only">Solo Salud de Hábitos</option>
              <option value="learning_days_only">Solo Días Adquiriendo</option>
              <option value="combined">Ambas (Combinadas en una tarjeta)</option>
      ...
      <Card title="Gestión de Datos (JSON)">
        <div className="p-3 mb-4 bg-yellow-50 dark:bg-yellow-700/20 border border-yellow-300 dark:border-yellow-600/30 rounded-md text-yellow-800 dark:text-yellow-200 text-sm flex items-start">
            <InformationCircleIcon className="w-5 h-5 mr-2 mt-0.5 flex-shrink-0 text-yellow-600 dark:text-yellow-400" />
            <div>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <p className={`text-xs text-[var(--color-warning)] bg-yellow-50 dark:bg-yellow-700/20 border border-yellow-200 dark:border-yellow-600/30 p-2 rounded-md mb-2`}>
